...
def main_driver():
    db = {}
    db[1] = create_patient_entry_dict("Ann", 'Abbles', 1, 34)
    db[2] = create_patient_entry_dict("Save", 'Help', 2, 45)
    db[3] = create_patient_entry_dict("dave", 'davis', 3, 50)
    print(db)
    print_database(db)
...
def print_database(db):
    for entry in db.values():
        print(entry)
        result = str(entry['MRN']) + ", Full Name: " + get_full_name(entry)
        + ", Age: " + str(entry['Age'])
        print(result)
